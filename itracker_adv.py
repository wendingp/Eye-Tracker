...
def preprocess(data):
    """Normalize and reshape the loaded data."""
...
class EyeTracker(object):
    def __init__(self):
        ...
        self.y = tf.placeholder(tf.float32, [None, 2], name='pos')  # Actual position of the gaze
        ...
        # Construct model and get the prediction
    ...
    def itracker_nets(self, eye_left, eye_right, face, face_mask, weights, biases):
        """Build the iTracker net model and return the output prediction."""
        # pathway: left eye
    ...
    def train(self, train_data, val_data, lr=1e-3, batch_size=128, max_epoch=1000, min_delta=1e-4, patience=10,
              print_per_epoch=10, out_model='my_model'):
        """Train the iTracker model"""
        ...
        # Define the loss as the MSE and the optimizer using Adam
        ...
        # Add validation nodes to collection
        ...
        with tf.Session() as sess:
            ...
            for n_epoch in range(1, max_epoch + 1):
                ...
                for batch_train_data in next_batch(train_data, batch_size):
                    ...
                    train_batch_loss, train_batch_err = sess.run([self.loss, self.err],
                ...
                for batch_val_data in next_batch(val_data, batch_size):
                    val_batch_loss, val_batch_err = sess.run([self.loss, self.err],
                ...
                if n_incr_error >= patience:
                    print 'Early stopping occurred. Optimization Finished!'
...
def validate_model(session, val_data, val_ops, batch_size=256):
    ...
    Args:
        ...
        batch_size: The batch size used in validation.
    Returns:
    ...
    val_err = 0
    for batch_val_data in next_batch(val_data, batch_size):
        val_batch_err = session.run(err, feed_dict={eye_left: batch_val_data[0], eye_right: batch_val_data[1],
                                                    face: batch_val_data[2], face_mask: batch_val_data[3],
                                                    y: batch_val_data[4]})
        val_err += val_batch_err / val_n_batches
    return val_err
...
def train(args):
    ...
    model = EyeTracker()
    train_loss_history, train_err_history, val_loss_history, val_err_history = model.train(train_data, val_data,
    ...
    if args.save_loss:
        with open(args.save_loss, 'w') as output_file:
            np.savez(output_file, train_loss_history=train_loss_history, train_err_history=train_err_history,
                     val_loss_history=val_loss_history, val_err_history=val_err_history)

    if args.plot_loss:
        plot_loss(np.array(train_loss_history), np.array(train_err_history), np.array(val_err_history), start=0, per=1,
...
def main():
    ...
    # Run training or validating
    if args.train:
        train(args)
    else:
        if not args.load_model:
            raise Exception('load_model arg needed in validation phase')
